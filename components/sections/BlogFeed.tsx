...
export default function BlogFeed() {
  ...
  return (
    <section id="feed" className="relative overflow-hidden bg-[linear-gradient(180deg,#070414,#0f1c33)] py-24 lg:py-28">
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-5">
        <div className="mb-8 flex flex-col items-start justify-between gap-4 rounded-[28px] border border-white/12 bg-white/[0.05] px-6 py-5 text-xs uppercase tracking-[0.32em] text-white/55 backdrop-blur-xl sm:flex-row sm:items-center">
        ...
        <WarmSpotlight intensity={0.7}>
          <motion.article
            className="mb-10 grid gap-4 rounded-[32px] border border-white/12 bg-white/[0.08] p-6 text-sm text-white/80 backdrop-blur-2xl md:grid-cols-[0.75fr_0.25fr]"
          ...
          >
            <div>
              <div className="flex flex-wrap items-center justify-between gap-3 text-xs uppercase tracking-[0.32em] text-white/60">
              ...
              <p className="mt-3 text-base leading-relaxed text-white/80">{headlinePost.content}</p>
            </div>
            <div className="flex flex-col justify-between rounded-3xl border border-white/15 bg-white/10 p-4 text-[11px] uppercase tracking-[0.35em] text-white/65">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {otherPosts.map((post, index) => {
            ...
            return (
              <WarmSpotlight key={post.id} intensity={0.64}>
                <motion.article
                  className="flex h-full flex-col justify-between rounded-[28px] border border-white/12 bg-white/[0.08] p-6 text-sm text-white/80 backdrop-blur-xl"
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.32em] text-white/65">
                        {author.name}
                      </div>
                      <p className="mt-2 text-xs uppercase tracking-[0.32em] text-white/45">{author.role}</p>
                    </div>
                    <div
                      className={`flex h-10 w-10 flex-none items-center justify-center rounded-full bg-gradient-to-br ${author.accent} text-xs font-semibold text-white`}
                    >
                      {author.name.slice(0, 2).toUpperCase()}
                    </div>
                  </div>

                  <p className="mt-4 text-base leading-relaxed text-white/85">{post.content}</p>

                  <div className="mt-6 flex items-center justify-between text-[11px] uppercase tracking-[0.32em] text-white/45">
